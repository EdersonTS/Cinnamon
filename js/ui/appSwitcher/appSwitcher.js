...
// some window that should be hidden | See Applets -> window-list@cinnamon.org
const HIDDEN_WINDOW = ["JavaEmbeddedFrame"];
...
function getWindowsForBinding(binding) {
    ...
    for (let i in windowActors) {
        let windowActor = windowActors[i].get_meta_window();

        if ( HIDDEN_WINDOW.indexOf(windowActor.get_title()) == -1 )
            windows.push(windowActor);
